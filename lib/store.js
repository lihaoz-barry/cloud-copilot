...
// Only for PROJECT_TYPES / isProjectType: the set of valid types must have one
// definition, or adding a third one here would be silently rejected on write.
...
/**
 * Manual project-type overrides (issue #70).
 *
 * Auto-detection reads the repo's layout, but the human looking at the badge
 * always wins. The override lives here rather than in localStorage on purpose:
 * it has to survive a browser change and be visible to the server, which is
 * what decides which verification commands a run is told to use.
 *
 * Shape: { "<repo>": "ios" | "web" }. A repo with no entry follows detection.
 */
function getProjectTypeOverrides() {
  const state = load();
  const o = (state.settings && state.settings.projectTypes) || {};
  return o && typeof o === 'object' ? { ...o } : {};
}

/** The stored override for one repo, or null when it follows auto-detection. */
function getProjectTypeOverride(repo) {
  const o = getProjectTypeOverrides();
  const v = o[repo];
  return repoConfig.isProjectType(v) ? v : null;
}

/** `type: null` clears the override so the repo goes back to auto-detection. */
function setProjectTypeOverride(repo, type) {
  if (typeof repo !== 'string' || !repo.trim()) throw new Error('repo must be a non-empty string');
  const clearing = type === null || type === undefined;
  if (!clearing && !repoConfig.isProjectType(type)) {
    throw new Error(`unknown project type "${type}"`);
  }
  const state = load();
  if (!state.settings.projectTypes) state.settings.projectTypes = {};
  if (clearing) delete state.settings.projectTypes[repo];
  else state.settings.projectTypes[repo] = type;
  save(state);
  return getProjectTypeOverride(repo);
}
