...
function startJob(key, { bin, args, cwd, meta, onSession, onDone, onProgress }) {
  ...
  try {
    ...
    child = spawn(bin, args, { cwd, env: process.env, detached: true });
  } catch (err) {
    job.status = 'done';
  ...
  child.on('close', async (code) => {
    ...
    scheduleReap(job);
    // Fire *after* result/done have been broadcast and `job.result` is final,
    // so settle handlers (the queue scheduler) see the authoritative status.
    const waiters = job._settleWaiters.splice(0);
    for (const cb of waiters) {
      try {
        cb(job);
      } catch {
        /* a settle handler must never break job completion */
      }
    }
  });
...
/**
 * Run `cb(job)` once the job has finished (immediately, on the next tick, if it
 * already has). Used by the queue scheduler to free the slot/repo lock and to
 * resolve dependent entries.
 */
function whenSettled(job, cb) {
  if (!job) return;
  if (job.status === 'done') {
    setImmediate(() => cb(job));
    return;
  }
  job._settleWaiters.push(cb);
}

module.exports = { startJob, subscribe, getJob, runningKeys, cancelJob, whenSettled };
