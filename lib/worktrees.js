...
function git(repoPath, args, timeout = 20000) {
  return execFileSync('git', ['-C', repoPath, ...args], {
    ...
    // Capture stderr instead of inheriting it: several calls here fail on
    // purpose (`worktree unlock` on an unlocked tree, `log` against a missing
    // ref) and their noise would otherwise be printed as if it were an error.
