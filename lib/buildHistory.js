...
// Only the tail of a transcript is scanned. `deploy.conversation` is a full
// Copilot CLI + fastlane log (routinely hundreds of KB), the fatal line is at
// the very end, and this runs for every failed build on every
// `/api/testflight/builds` request over an intentionally unbounded history —
// scanning whole transcripts would make the page cost grow with total log size.
const SCAN_TAIL_BYTES = 32 * 1024;
 ...
 */
function summarizeFailure(text, { maxLen = MAX_REASON_LEN, tailBytes = SCAN_TAIL_BYTES } = {}) {
  if (typeof text !== 'string' || !text.trim()) return null;
  let scan = text;
  if (scan.length > tailBytes) {
    scan = scan.slice(-tailBytes);
    // The cut lands mid-line; drop that fragment so half a sentence can never
    // be reported as the reason (unless the tail is one single long line).
    const nl = scan.indexOf('\n');
    if (nl !== -1) scan = scan.slice(nl + 1);
  }
  const lines = scan
    .split(/\r?\n/)
