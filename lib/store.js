...
const buildHistory = require('./buildHistory');

// `CC_DATA_DIR` lets tests point the store at a throwaway directory instead of
// the real state file; unset in normal runs.
...
function blankDeploy() {
  return {
    ...
    version: null,
    // Code this attempt shipped, pinned when the deploy starts so the build
    // history keeps pointing at the right commit even after the PR moves on.
    branch: null,
    commit: null, // { sha, abbrev, committedDate, headline, url }
...
function migrate(r, repo, issueNumber) {
  ...
  for (const pr of Object.values(r.prs)) {
    ...
    // v7: backfill the per-attempt branch/commit pin on the live deploy and on
    // every archived attempt, so build history rows have the fields even when
    // the value stays null for pre-existing builds.
    for (const d of [pr.deploy, ...(pr.deployHistory || [])]) {
      if (!d) continue;
      // Backfilled independently: a record that somehow carries one of the two
      // must not have the other wiped by the migration.
 ...
 *
 * `action` is one of LIVE_STATUS's keys; `prNumber` is required for everything
 * except 'work'.
 */
function forceAbort(repo, issueNumber, action, prNumber = null) {
  ...
  // Never CREATE a record on the way to aborting one. updateRecord() writes a
  // blank record for any key it is handed, so without this an unauthenticated
  // `POST .../issues/99999/work/cancel` would litter state.json with rows for
  // issues that were never run here.
  if (!load().issues[key(repo, n)]) return false;
  let changed = false;
  updateRecord(repo, n, (r) => {
    const pr = action === 'work' ? null : r.prs[Number(prNumber)];
    const target = action === 'work' ? r.work : pr && pr[action];
    if (!target || target.status !== live) return;
    target.status = 'aborted';
    target.finishedAt = new Date().toISOString();
    target.conversation = (target.conversation || '') + ABORT_NOTE(ABORT_LABEL[action] || action);
    changed = true;
    // updateRecord already recomputes work.durationMs; the PR-scoped slots are
    // ours to keep honest.
    if (pr) target.durationMs = durationOf(target);
  });
  return changed;
}
...
/**
 * Does this PR record hold anything cloud-copilot itself produced?
 *
 * Everything else on a PR record is cached GitHub metadata that can be
 * re-fetched at will, but a deploy attempt (live or archived), a merge or a
 * chat log exists ONLY here — `listAllBuilds()` reads exactly those archived
 * attempts for the TestFlight build history, so deleting such a record
 * permanently rewrites that history.
 */
function hasLocalHistory(pr) {
  if (!pr) return false;
  return Boolean(
    (pr.deployHistory && pr.deployHistory.length > 0) ||
      (pr.deploy && pr.deploy.status && pr.deploy.status !== 'idle') ||
      (pr.merge && pr.merge.status && pr.merge.status !== 'idle') ||
 ...
 *
 * A record carrying local history is kept regardless: the scan behind
 * `validPrNumbers` comes from `gh pr list --limit 100`, so on a busy repo an
 * older PR simply falls out of the window and would otherwise be deleted here —
 * silently erasing every build it ever shipped from the TestFlight page. Same
 * rule `pruneClosedPrs` applies, for the same reason.
 */
function pruneStaleGhPrs(repo, issueNumber, validPrNumbers) {
  return updateRecord(repo, issueNumber, (r) => {
    ...
    for (const k of Object.keys(r.prs)) {
      const pr = r.prs[k];
      if (pr.source !== 'gh' || valid.has(pr.prNumber)) continue;
      if (!hasLocalHistory(pr)) delete r.prs[k];
    }
  });
}
...
const TERMINAL_DEPLOY = ['success', 'failed', 'aborted'];
...
/**
 * Archive the PR's current deploy attempt into `deployHistory` before it is
 * replaced, so no build ever disappears from the TestFlight history.
 *
 * An attempt that never ran (`idle`) has nothing to remember. A terminal
 * attempt is always archived, even if it somehow carries no timestamps: it ran,
 * so it is a build, and dropping it would rewrite history.
 *
 * `interrupted` is passed only by `startNewDeploy`, the one caller that can
 * prove a still-`deploying` attempt will never report a result: a new attempt
 * cannot start while its job is alive (the deploy route holds the repo's
 * working-tree lock), so such a record is left over from a crash/restart and is
 * archived as `aborted` with a note instead of being dropped. Every other
 * caller leaves a non-terminal attempt alone — it is still running and still
 * owns `pr.deploy`, and archiving it would invent a build that never ended.
 */
function archiveDeploy(pr, { interrupted = false } = {}) {
  const d = pr.deploy;
  if (!d || !d.status || d.status === 'idle') return;
  const terminal = TERMINAL_DEPLOY.includes(d.status);
  if (!terminal) {
    if (!interrupted) return;
    if (!d.startedAt && !d.finishedAt) return; // never actually got going
  }
  const snapshot = { ...d };
  if (!terminal) {
    snapshot.status = 'aborted';
    snapshot.conversation = `${snapshot.conversation || ''}\n\n[interrupted: this deploy never reported a result (the server most likely restarted mid-run) and was superseded by a newer attempt]`.trim();
    // Only an interrupted attempt gets a synthetic finish time — "now" is when
    // we noticed it was gone, which is the closest honest answer and lets it
    // sort. A terminal attempt that simply never recorded one keeps its null:
    // stamping today's date would make an old build look brand new.
    if (!snapshot.finishedAt) snapshot.finishedAt = new Date().toISOString();
  }
  snapshot.durationMs = durationOf(snapshot);
  if (!pr.deployHistory) pr.deployHistory = [];
  pr.deployHistory.push(snapshot);
}

/**
 * Start a fresh deploy attempt for a PR, archiving the previous one into
 * `deployHistory` first — so every build number/version a PR has ever shipped
 * stays visible, not just the latest.
 */
function startNewDeploy(repo, issueNumber, prNumber) {
  return updateRecord(repo, issueNumber, (r) => {
    ...
    if (!r.prs[n]) r.prs[n] = blankPr(n);
    const pr = r.prs[n];
    archiveDeploy(pr, { interrupted: true });
 ...
 *
 * A deploy that is still running is the one exception. This runs from the
 * `onDone` of chat/review/update turns, which are allowed to run concurrently
 * with a Deploy (they hold their own worktree, not the repo's working tree), so
 * "new commits landed" says nothing about whether the deploy finished. Touching
 * a live attempt would archive a build that never ended and throw away the
 * branch/commit this deploy pinned when it started — while its own `onDone`
 * still has to write the result somewhere. Leave it to finish; the Merge lock
 * is reset either way.
 */
function resetForNewCommits(repo, issueNumber, prNumber) {
  return updateRecord(repo, issueNumber, (r) => {
    ...
    if (!r.prs[n]) r.prs[n] = blankPr(n);
    const pr = r.prs[n];
    const live = pr.deploy && pr.deploy.status && pr.deploy.status !== 'idle' && !TERMINAL_DEPLOY.includes(pr.deploy.status);
    if (!live) {
      archiveDeploy(pr);
      pr.deploy = blankDeploy();
    }
 ...
 *
 * Every attempt stays its own entry — several builds of the same version are
 * never collapsed. Returns `{ builds, errors }`: a record that cannot be read
 * lands in `errors` instead of failing the whole listing.
 */
function listAllBuilds() {
  ...
  for (const k of Object.keys(state.issues)) {
    const raw = state.issues[k];
    try {
      const { record } = migrate(raw, raw && raw.repo, raw && raw.issueNumber);
      records.push(record);
    } catch (err) {
      // One unreadable issue record must not take the whole history down.
      errors.push({ repo: (raw && raw.repo) || k, issueNumber: (raw && raw.issueNumber) ?? null, message: err.message });
    }
  }
  const flat = buildHistory.flattenBuilds(records);
  return { builds: flat.builds, errors: [...errors, ...flat.errors] };
}
