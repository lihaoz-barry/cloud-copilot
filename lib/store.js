 ...
 *                       exitCode, startedAt, finishedAt, durationMs },
 *             mergeMain: { status, headRef, baseRef, conversation, exitCode,
 *                          startedAt, finishedAt, durationMs },
...
// "Merge main" — the OPPOSITE direction of `merge`: it merges the PR's base
// branch (usually main) INTO the PR's head branch and pushes, so a PR that has
// fallen behind can be brought up to date without dropping to a terminal.
function blankMergeMain() {
  return {
    status: 'idle', // idle | merging | success | conflict | failed | aborted
    headRef: null, // PR branch that received the merge
    baseRef: null, // branch that was merged in (usually main)
    conversation: '',
    exitCode: null,
    startedAt: null,
    finishedAt: null,
...
/**
 * Patch a specific PR's "merge main" sub-record (base branch → PR branch) and
 * recompute its duration. `patch` is (mergeMain, pr) => void.
 */
  ...
  for (const n of issueNumbers) {
    ...
    out[n] = {
      ...
      prs: prsArray(record).map((p) => ({
        ...
        mergeMain: {
          status: p.mergeMain.status,
          headRef: p.mergeMain.headRef,
          baseRef: p.mergeMain.baseRef,
          startedAt: p.mergeMain.startedAt,
          finishedAt: p.mergeMain.finishedAt,
          durationMs: p.mergeMain.durationMs,
          hasConversation: Boolean(p.mergeMain.conversation),
        },
      })),
    };
  }
