...
/**
 * Rewrite one cached issue in place, e.g. after a label was toggled.
 *
 * The alternative — invalidating the whole `issues` entry — would blank the
 * labels /statuses serves until the next full `gh issue list`, which is exactly
 * the moment the committed checkbox most needs to be right. `at` is left alone
 * on purpose: the rest of the entry is precisely as old as it was, and renewing
 * it would postpone a real refresh.
 *
 * @param {(issue: object) => object|null} fn  receives a copy, returns the
 *   replacement (or null to leave the cache untouched)
 * @returns {boolean} whether an issue was found and rewritten
 */
function updateIssue(ownerRepo, issueNumber, fn) {
  if (!loaded) load();
  const entry = state.repos[ownerRepo] && state.repos[ownerRepo].issues;
  if (!entry || !Array.isArray(entry.data)) return false;
  const n = Number(issueNumber);
  const idx = entry.data.findIndex((i) => i && i.number === n);
  if (idx < 0) return false;
  const next = fn({ ...entry.data[idx] });
  if (!next) return false;
  entry.data = entry.data.map((issue, k) => (k === idx ? next : issue));
  persist();
  return true;
}
...
module.exports = { TTL_MS, CACHE_FILE, load, get, set, updateIssue, invalidate, syncedAt };
