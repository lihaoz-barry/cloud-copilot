...
/**
 * The GitHub label that upgrades one issue from "open a PR" to "open a PR and
 * ship it" (issue #70): after the PR exists, deploy from the worktree. Only
 * offered for iOS repos, where TestFlight happily takes several builds in
 * parallel; a Web service can only have one build live at a time.
 */
const COMMIT_DEPLOY_LABEL = process.env.CC_COMMIT_DEPLOY_LABEL || 'commit and deploy';
 ...
 */
async function setIssueLabel(ownerRepo, issueNumber, label, on, meta = {}) {
  if (!ownerRepo) throw new Error('not a GitHub repo');
  if (on) {
    try {
      await run(GH_BIN, [
        ...
        meta.color || '0E8A16',
        '--description',
        meta.description || 'cloud-copilot drives this issue to a finished PR automatically',
      ]);
    } catch {
