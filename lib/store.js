...
/**
 * "Merge main" = the OPPOSITE direction from `merge`: it merges the PR's base
 * branch (usually main) INTO the PR's head branch and pushes, so a long-lived
 * PR can catch up with main without dropping to a terminal.
 */
function blankMergeMain() {
  return {
    status: 'idle', // idle | merging | success | conflict | failed | aborted
    conversation: '',
    exitCode: null,
    headRef: null, // PR branch that main was merged into
    baseRef: null, // branch that was merged in (the PR's base)
    conflictFiles: [], // populated when status === 'conflict'
    startedAt: null,
    finishedAt: null,
    durationMs: null,
  };
...
function migrate(r, repo, issueNumber) {
  ...
  for (const pr of Object.values(r.prs)) {
    ...
    // v5: backfill the "Merge main" (base → head) sub-record.
    if (!pr.mergeMain) {
      pr.mergeMain = blankMergeMain();
      changed = true;
    }
  }
...
/**
 * Patch a specific PR's "merge main" sub-record and recompute its duration.
 * `patch` is (mergeMain, pr) => void.
 */
function updateMergeMain(repo, issueNumber, prNumber, patch) {
  return updateRecord(repo, issueNumber, (r) => {
    const n = Number(prNumber);
    if (!r.prs[n]) r.prs[n] = blankPr(n);
    if (!r.prs[n].mergeMain) r.prs[n].mergeMain = blankMergeMain();
    patch(r.prs[n].mergeMain, r.prs[n]);
    r.prs[n].mergeMain.durationMs = durationOf(r.prs[n].mergeMain);
  });
}
  ...
  for (const n of issueNumbers) {
    ...
    out[n] = {
      ...
      prs: prsArray(record).map((p) => ({
        ...
        mergeMain: {
          status: (p.mergeMain && p.mergeMain.status) || 'idle',
          headRef: p.mergeMain && p.mergeMain.headRef,
          baseRef: p.mergeMain && p.mergeMain.baseRef,
          conflictFiles: (p.mergeMain && p.mergeMain.conflictFiles) || [],
          startedAt: p.mergeMain && p.mergeMain.startedAt,
          finishedAt: p.mergeMain && p.mergeMain.finishedAt,
          durationMs: p.mergeMain && p.mergeMain.durationMs,
          hasConversation: Boolean(p.mergeMain && p.mergeMain.conversation),
        },
      })),
    };
  }
...
function startAdminTurn(turnId, { userText, mode, repo, images, sessionId }) {
  if (!turnId) return;
  const state = load();
  const now = new Date().toISOString();
...
function appendPreIssueChatMessage(repo, id, { role, text }) {
  return updatePreIssue(repo, id, (pre) => {
    if (!pre.chat) pre.chat = { sessionId: null, messages: [] };
    pre.chat.messages.push({ role, text, at: new Date().toISOString() });
  });
}
