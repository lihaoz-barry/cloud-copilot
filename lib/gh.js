 ...
 * from the PR title). Returns { number, headRefName, baseRefName, url, title }
 * or null on failure.
 */
...
/**
 * Ensure a label exists in the repo so `gh issue create --label` can't fail on
 * a fresh repo. Creating an existing label is a harmless no-op error we swallow.
 */
async function ensureLabel(ownerRepo, name, { color = 'BFD4F2', description = '' } = {}) {
  try {
    await run(GH_BIN, [
      'label', 'create', name,
      '--repo', ownerRepo,
    ...
    /* already exists (or no permission) — issue creation degrades gracefully */
  }
}

const LABEL_DESCRIPTIONS = {
  'ui-validation': 'Requires headless-browser UI validation (ui-test skill)',
};
 ...
 */
async function createIssue(ownerRepo, title, body, labels = []) {
  if (!ownerRepo) throw new Error('not a GitHub repo');
  const clean = (Array.isArray(labels) ? labels : [])
    .filter((l) => typeof l === 'string' && /^[\w.-]{1,50}$/.test(l));
  for (const label of clean) {
    await ensureLabel(ownerRepo, label, { description: LABEL_DESCRIPTIONS[label] || '' });
  }
  const args = [
    'issue',
    'create',
  ...
  for (const label of clean) args.push('--label', label);
  let out;
  try {
    out = await run(GH_BIN, args);
  } catch (err) {
    // A label the repo rejects must never cost the user their drafted issue.
    if (!clean.length) throw err;
    out = await run(GH_BIN, args.slice(0, 8));
  }
