 ...
 *
 * A `shell` deploy may additionally describe cloud-copilot deploying *itself*
 * (see lib/selfDeploy.js), which is the only case where one deploy has to
 * restart two processes:
 *
 *   { "deploy": { "type": "shell", "command": "npm run cc:restart",
 *                 "selfDeploy": true,
 *                 "scheduler": { "command": "npm run cc:restart-scheduler",
 *                                "paths": ["scheduler-server.js", "lib/supervisor.js"] } } }
 *
...
function loadDeployConfig(repoPath) {
  ...
  if (deploy && VALID_TYPES.has(deploy.type)) {
    ...
    return { type: deploy.type, command: deploy.command, deploy, raw };
  }
