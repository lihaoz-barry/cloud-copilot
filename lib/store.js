 ...
 *
 * An attempt that never ran (`idle`) has nothing to remember. An attempt that
 * ran but never reported a result — status still `deploying` because the server
 * was restarted or crashed mid-run, and the in-memory job is gone — used to be
 * dropped silently, losing that build; it is archived as `aborted` with a note
 * instead.
 */
function archiveDeploy(pr) {
  const d = pr.deploy;
  if (!d || !d.status || d.status === 'idle') return;
  if (!d.startedAt && !d.finishedAt) return;
  const snapshot = { ...d };
  if (!TERMINAL_DEPLOY.includes(snapshot.status)) {
