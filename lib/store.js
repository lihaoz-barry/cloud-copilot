 ...
 *             chat: { sessionId, messages: [{ role, text, mode, at }] }
 *           }
 *         }
 *       }
 *     }
...
function load() {
  ...
  try {
    ...
    return { issues: {}, dismissed: {}, adminChats: {}, preIssues: {} };
  }
}
 ...
 */
function appendChatMessage(repo, issueNumber, prNumber, { role, text, mode }) {
  return updateRecord(repo, issueNumber, (r) => {
    const n = Number(prNumber);
    if (!r.prs[n]) r.prs[n] = blankPr(n);
    if (!r.prs[n].chat) r.prs[n].chat = blankChat();
    r.prs[n].chat.messages.push({ role, text, mode, at: new Date().toISOString() });
  });
}
 ...
 */
function appendAdminTurn(sessionId, { userText, assistantText, mode, repo }) {
  ...
  chat.messages.push({ role: 'user', text: userText, mode, repo, at: now });
