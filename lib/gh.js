...
const { isWorktreeContainerName } = require('./worktree');
...
function listRepos(root) {
  ...
  for (const e of entries) {
    ...
    // `<repoName>-worktrees/` holds the per-issue git worktrees used for
    // concurrent Create PR runs. Today the container has no `.git` of its own
    // (the real worktrees live one level deeper), so it would already be
    // skipped — but reject it by name too, so making this scan recursive later
    // can't silently start listing worktrees as standalone repos.
    if (isWorktreeContainerName(e.name)) continue;
