 ...
 * not vanish because a checkout was moved or deleted. A repo whose deploy
 * config can't be read keeps its builds too, and is reported once in `errors`.
 */
function annotateBuilds(builds, { resolveRepo, deployTypeOf }) {
  ...
  const infoFor = (name) => {
    ...
    try {
      ...
      info = { repo: repo || null, type: repo ? deployTypeOf(repo) : null, error: null };
    } catch (err) {
      info = { repo: null, type: null, error: err.message };
    }
    if (info.error) errors.push({ repo: name || null, issueNumber: null, prNumber: null, message: info.error });
    cache.set(name, info);
    return info;
  };
