...
function appendChatMessage(repo, issueNumber, prNumber, { role, text, mode, images, model }) {
  return updateRecord(repo, issueNumber, (r) => {
    ...
    r.prs[n].chat.messages.push({
      ...
      text,
      mode,
      // The model this turn actually ran with — chats can pick one per turn, so
      // a transcript can legitimately mix models and the UI badges each bubble.
      model: model || null,
...
function appendAdminTurn(sessionId, { userText, assistantText, mode, repo, images, model }) {
  ...
  chat.messages.push({ role: 'user', text: userText, mode, repo, model: model || null, images: Array.isArray(images) ? images : [], at: now });
