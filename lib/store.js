...
// Notification center.
//
// Actions now run through a queue (lib/queue.js), so most of them finish while
// the user is looking at something else entirely — or at nothing at all, with
// the phone in a pocket. Terminal outcomes are therefore recorded here as
// durable, server-side notifications the UI renders in its top-left bell.
// They are explicitly *not* ephemeral toasts: a queued Deploy that got
// cancelled because its Create PR failed must still be discoverable minutes
// later. The user dismisses each one after reading it (or clears them all).
// ---------------------------------------------------------------------------

const MAX_NOTIFICATIONS = 100;

/**
 * Record a notification. `kind` drives the icon/colour in the UI:
 * 'success' | 'failure' | 'cancelled' | 'info'.
 */
function addNotification({ kind, title, message, repo, issueNumber, prNumber, action, key }) {
  const state = load();
  if (!Array.isArray(state.notifications)) state.notifications = [];
  const note = {
    id: crypto.randomUUID(),
  ...
  // Newest-first, hard-capped so the state file can't grow without bound.
...
/** All notifications, newest first, plus the unread count for the badge. */
...
/** Mark one as read (the UI does this when the panel is opened). */
...
/** Delete a single notification — "已读之后可以把它删除掉". */
