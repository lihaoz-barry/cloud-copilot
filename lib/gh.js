...
/**
 * Title of an issue, from the in-memory cache only — never a network call.
 * Used to give notifications a human subject line ("Create PR · repo#27" with
 * the issue's title in the body) without slowing down or failing a job when
 * GitHub is unreachable. Returns null on a cache miss.
 */
function cachedIssueTitle(ownerRepo, issueNumber) {
  const cached = issueCache.get(ownerRepo);
  if (!cached) return null;
  const found = cached.issues.find((i) => Number(i.number) === Number(issueNumber));
