...
async function decide(repo, issue, record) {
  ...
  const pr = open[0];
  let sync = pr.sync;
  // No comparison on record yet — a PR opened since the last three-minute
  // sweep, most often the one this loop just created. Falling through to
  // `review` here would spend a whole agent run reviewing a diff that is about
  // to be rewritten by the base merge it actually needs, so ask git now. Fork
  // PRs legitimately have no local answer; those still fall through.
  if (!sync && deps.syncRepo) {
    try {
      await deps.syncRepo(repo);
      const fresh = deps.store.getRecord(repo.name, issue.number).prs || {};
      sync = (fresh[String(pr.prNumber)] || {}).sync;
    } catch {
      /* best effort — an unanswerable PR is handled below */
    }
  }
