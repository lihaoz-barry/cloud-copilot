...
class Job {
  constructor(key, { bin, args, cwd, meta }) {
    ...
    // Resolves (never rejects) with this job once it reaches `done`. Lets a
    // server-side caller — the task-queue worker — await a run the same way an
    // SSE subscriber watches one, without polling. HTTP callers ignore it.
 ...
 * @param {string} key
 * @param {object} opts { bin, args, cwd, env, meta, onSession, onDone, onProgress }
 *   env: extra environment variables merged over process.env — the task queue
 *     uses it to hand a worktree run its own PORT so a dev server started there
 *     can't collide with the main instance
 ...
 * @returns {Job}
 */
function startJob(key, { bin, args, cwd, env, meta, onSession, onDone, onProgress }) {
