...
/**
 * Environment handed to every job's child process.
 *
 * `CLOUD_COPILOT_JOB=1` marks the run as "started by cloud-copilot" so the
 * Copilot CLI's generic `sessionEnd` ntfy hook can bow out (issue #27): this
 * server sends its own, far more specific push when the job finishes, and two
 * notifications per job — one of them content-free — is worse than one.
 */
function childEnv() {
  return { ...process.env, CLOUD_COPILOT_JOB: '1' };
}

/**
 * Push a task-aware notification for a job that just reached a terminal state.
 * Everything the notifier needs is already on the job: `meta` (set at spawn
 * time) plus whatever `onDone` returned, including an optional `notify` blob
 * with extras like the issue/chat title. Best-effort — never awaited, never
 * allowed to throw.
 */
function announce(job) {
  try {
    ...
    notifier.notifyJobFinished({
      key: job.key,
      transcript: job.conversation,
      ...job.meta,
      action: result.action || job.meta.action,
      status:
        result.status ||
        (job.cancelled ? 'aborted' : job.exitCode === 0 ? 'success' : 'failed'),
      prNumber: result.prNumber != null ? result.prNumber : job.meta.prNumber,
    ...
    /* notifications must never affect the job */
  }
}
 ...
 * @returns {Job}
 */
function startJob(key, { bin, args, cwd, meta, onSession, onDone, onProgress }) {
  ...
  try {
    ...
    child = spawn(bin, args, { cwd, env: childEnv(), detached: true });
  } catch (err) {
    job.status = 'done';
    job.finishedAt = new Date().toISOString();
    job.conversation += `\n[spawn error] ${err.message}\n`;
    job.result = { action: job.meta.action, status: 'failed', error: `Failed to spawn "${bin}": ${err.message}` };
