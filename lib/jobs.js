...
function absorbReplay(job) {
  ...
  job._pending = '';
  if (replayed.length <= job.conversation.length) return;
  if (replayed.startsWith(job.conversation)) {
    appendTranscript(job, replayed.slice(job.conversation.length));
  } else {
    job.conversation = '';
...
function follow(job, sessionId) {
  ...
  job._detach = supervisorClient.streamSession(sessionId, {
    ...
    onChunk: (d) => {
      ...
      else appendTranscript(job, text);
    },
    onSession: (d) => {
...
function startLocal(job) {
  ...
  child.on('close', (code) => finishJob(job, { exitCode: code }));
}
 ...
 *
 * @param {string} key
 * @param {object} opts { bin, args, cwd, meta, env, onSession, onDone, onProgress }
...
function startJob(key, { bin, args, cwd, meta, env, onSession, onDone, onProgress }) {
  ...
  supervisorClient
    .spawnSession({
      ...
      meta: { ...job.meta, key, startedBy: 'dashboard' },
    })
    .then((out) => {
    ...
    .catch((err) => {
      ...
      console.warn(
        `[jobs] the supervisor on ${supervisorClient.BASE} could not start ${key}: ${err.message}` +
